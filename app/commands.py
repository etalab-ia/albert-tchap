...
async def fomo(room: MatrixRoom, message: Event, matrix_client: MatrixClient):
    ...
    event_parser.do_not_accept_own_message()
    missing_out_user = user_name_to_non_hl_user(
        event_parser.command("fomo").strip() or event_parser.sender_username()
    )
    salons = matrix_client.get_non_private_rooms()
    missing_out_salons = {
        salon_item.name: get_salon_moderators(
            salon_item, fomo_user_name=missing_out_user
        )
        for salon_item in salons.values()
    }
    message = (
        f"Les salons **géniaux** que {missing_out_user} loupe sont :\n - "
        + "\n - ".join(
            [
...
async def kick(room: MatrixRoom, message: Event, matrix_client: MatrixClient):
    ...
    event_parser.do_not_accept_own_message()
    maybe_user_name = user_name_to_non_hl_user(
        event_parser.command("kick").strip() or event_parser.sender_username()
    )

    salons = matrix_client.get_non_private_rooms()
    kick_out_salons = {
        salon_item.name: get_salon_moderators(
            salon_item, kick_user_name=maybe_user_name
        )
        for salon_item in salons.values()
    }
